...
def get_valuation_measures(symbols):
    ...
    for symbol in symbols:
        ...
        # 
        # 目的の列のみを抽出
        subset_v = valuation_measures_t.loc[['asOfDate', 'PbRatio', 'PeRatio']]
...
def get_data(months, tickers, column, flg):
    ...
    if flg == 1:
        ...
        # 列ラベルを日付に変更
        df.columns = dates
        # df = df[(df['PbRatio'] > df['PbRatio'].quantile(0.01)) & (df['PbRatio'] < df['PbRatio'].quantile(0.99))]
...
select_symbols_df = pd.DataFrame(columns=['証券コード', '社名', '時価総額', '予想PER', 'PER', 'PBR', '配当性向', "is_widget"])
    ...
    try:
        ...
        try:
            ...
            if trailing_annual_dividend_yield is not None:
                dividend_payout_ratio = round(trailing_annual_dividend_yield * 100, 2)
            else:
        ...
        new_row = pd.Series({"証券コード": stock_data['証券コード'], "社名": stock_data['社名'], "時価総額": stock_data['時価総額'], "予想PER": stock_data['予想PER'], "PER": stock_data['PER'], "PBR": stock_data['PBR'], "配当性向": stock_data['配当性向'], "is_widget": True})
        select_symbols_df = select_symbols_df.append(new_row, ignore_index=True)
    except:
...
try:
    ...
    tickers_close_value = get_data(months, tickers, "Close", 0)
    ...
    chart = (
        ...
        .encode(
            x="Date:T",
            y=alt.Y("Prices(YEN):Q", stack=None, scale=alt.Scale(domain=[ymin, ymax])),
            color='Name:N',
        ) 
        .configure_axis(
            gridOpacity=0.2,
    ...
    subsets = get_valuation_measures(tickers)
    charts_pbr = []
    charts_per = []
    for subset in subsets:
        chart_pbr = alt.Chart(subset).mark_line().encode(
            x='asOfDate',
            y=alt.Y('PbRatio', scale=alt.Scale(domain=[subset['PbRatio'].min()-0.1, subset['PbRatio'].max()+0.1])),
        ...
        chart_per = alt.Chart(subset).mark_line().encode(
            x='asOfDate',
            y=alt.Y('PeRatio', scale=alt.Scale(domain=[subset['PeRatio'].min()-0.1, subset['PeRatio'].max()+0.1])),
            color='symbol:N',  # 列 'symbol' をカラーに設定
    ...
    df_income = get_data(months, tickers, "Dividends", 1)
